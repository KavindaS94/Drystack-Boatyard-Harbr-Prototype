...
4. **Boat onto the rack** — Calendar **Osprey** (in the water) → Launch board **08:00 Lift** → Start → Done → **Stored**. Then **Create draft invoice** (Lift, banner **This invoice → Marina**). After that, launch from a boat already on the rack (Pelican portal).
5. **Busy Saturday** — Launch board date 15 Aug, ~50 tasks → Start → Done → status stored → launched → set departed → mark lift done → stored. Then Calendar → Pelican → **Create draft invoice** (Launch + Lift, banner **This invoice → Marina**). Monthly rack storage is not on that draft.
6. **Settings** — rename Dockyard / Dry stack; toggle auto do-not-launch; add a job type/product.
7. **Customer portal round trip** — open [/portal/demo-pelican-portal](http://localhost:5173/portal/demo-pelican-portal) as Priya (Pelican) → request a launch → staff Launch board (14 Aug) → Approve → Start → Done → portal shows **In the water**. Or from a reservation: **Send status link** → Open as customer.
