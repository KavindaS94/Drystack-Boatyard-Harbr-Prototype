...
export const DEMO_ROUTINE_GROUPS: { id: DemoRoutineGroup; title: string; intro: string }[] = [
  {
    id: "dry",
    title: "Dry stack",
    intro: "The boat lives on a rack or hardstand pad. Launch her into the water from Dry stack or Hardstand — not from the Calendar.",
  },
  {
    id: "yard",
    title: "Boatyard",
    intro: "Pads on land for repair. Book onto a pad from Dockyard, finish the job, then launch back to the water.",
  },
  {
    id: "office",
    title: "Office",
...
export const DEMO_ROUTINES: DemoRoutine[] = [
  {
    ...
    why: "Pelican is already on a rack. Launch her into the water from Dry stack Today.",
    openLabel: "Open Pelican",
    to: "/operations/dry-stack?tab=occupancy&script=rack",
    extra: { label: "Open Dry stack Today", href: "/operations/dry-stack?script=rack" },
    steps: [
  ...
  },
  {
    ...
    why: "Pelican is already on the rack. Staff take a phone or email request, check the fork-lift slot, then Approve.",
    openLabel: "Open Dry stack",
    to: "/operations/dry-stack",
    extra: { label: "Open fork lift", href: "/operations/dry-stack?tab=fork-lift" },
    steps: [
      "Dry stack → Today → Log request. Search Pelican, Launch, pick a free fork-lift slot → Save. Customer is emailed.",
      "Requests — Pelican. Approve. She moves onto the Launch tab. Start, then Done.",
  ...
  },
  {
    ...
    steps: [
      "Dry stack → Racks → Tern (Mark Chen): red Account overdue. Today Start stays grey.",
  ...
  },
  {
    id: "hardstand",
    group: "dry",
    title: "Hardstand: ground storage",
    why: "Gannet sits on HS1. Same launch/lift as dry stack, fork lift, not stacked.",
    openLabel: "Open Gannet",
    to: "/operations/hardstand?tab=occupancy&boat=Gannet",
    extra: { label: "Open Hardstand Today", href: "/operations/hardstand" },
    steps: [
      "Hardstand → Pads → Gannet on HS1.",
      "Hardstand → Today → Requests: Friday launch request. Approve (fork-lift slot held, customer emailed).",
      "Hardstand → Pads → click an empty HS pad to book a boat that is not on a water berth.",
      "Turn Hardstand off in Settings → Modules & words: the Hardstand menu disappears. Dry stack fork lift stays.",
    ],
  },
  {
    id: "saturday",
  ...
  },
  {
    ...
    steps: [
      "Dockyard → Jobs or Pads → Sea Sprite. Job is open (checklist, hours, materials).",
  ...
  },
  {
    ...
    openLabel: "Open Dockyard pads",
    to: "/operations/boatyard?tab=occupancy",
    extra: { label: "Open yard crew", href: "/yard" },
    steps: [
      "Dockyard → Pads → click empty H5. Pick a boat that is not on a water berth (Gannet). Job type Engine service, lift 09:00. Confirm.",
      ...
      "Every job lifts first, then the work, then launch. Travel lift tab shows the booked Lift. When the job is ready: Schedule launch / Move launch — slot is checked, customer emailed.",
  ...
  },
  {
    ...
    openLabel: "Open Sea Sprite",
    to: "/operations/boatyard?tab=occupancy&script=yard-job",
    steps: [
      "Dockyard → Pads → Sea Sprite → Move.",
  ...
  },
  {
    ...
    steps: [
      "Dockyard → Jobs → Kingfisher. Job type is Engine service. Who does the work = Contractor (Marine Works). Notify sends a simulated email.",
  ...
  },
  {
    ...
    extra: { label: "Open Dockyard pads", href: "/operations/boatyard?tab=occupancy" },
    steps: [
      "Dry stack → Racks → click empty DS7. Pick a boat that is not on a water berth → Confirm.",
      "Dockyard → Pads → empty H5. Job type required. Occupied pads show Reservation Conflict.",
  ...
  },
  {
    ...
    steps: [
      "Modules & words — independently hide Boatyard, Dry stack, or Hardstand. That module disappears from Operations. Equipment tab sets travel-lift / fork-lift hours and slot length.",
